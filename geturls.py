...
for link in soup.find_all('a'):	#Se buscan todos los enlaces de la web
	if url in link.get('href'):	#Se utiliza para buscar solo enlaces al propio dominio, para evitar ser intrusivo en paginas externas enlazadas
		if "=" in link.get('href'):	#Se busca solo las url que contengan informacion modificable en la url
